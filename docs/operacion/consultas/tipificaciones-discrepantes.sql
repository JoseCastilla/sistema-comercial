  ...
  SELECT
    ...
    CASE
      ...
      WHEN resultado = 'SIN_RESPUESTA'
        AND (obs LIKE '%llamar manana%' OR obs LIKE '%llamar mas tarde%' OR obs LIKE '%llamar a las%' OR obs LIKE '%volver a llamar%' OR obs LIKE '%agend%' OR obs LIKE '%cita%')
...
SELECT
  to_char(c.created_at AT TIME ZONE 'America/Lima', 'DD/MM HH24:MI') AS registrada_lima,
  u.name                                                              AS asesor,
  ...
  c.observation
FROM clasificados c
JOIN recovery_cases rc ON rc.id = c.case_id
JOIN recovery_case_attempts a ON a.id = c.id
JOIN users u ON u.id = a.actor_user_id
WHERE c.discrepancia IS NOT NULL
ORDER BY c.created_at DESC;

-- Si `unaccent` no está instalada: CREATE EXTENSION IF NOT EXISTS unaccent;
-- (solo lectura de texto; no toca datos).
