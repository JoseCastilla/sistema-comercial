...
export async function getPerformanceReconciliation(
  ...
  const teamOptions = await database.commercialTeam.findMany({
  ...
  return {
    ...
    totals: {
      ...
      baseCommissionCents: payablePostpaid * 2_500 + payablePrepaid * 1_250,
    },
    lines: visible.map(({ order, evaluation }) => ({
      ...
      baseCommissionCents: evaluation.baseCommissionCents,
    })),
    pagination: {
      page,
      pageSize,
